...
from nerte.nerte_cpp import intersection_ray_depth as cpp_intersection_ray_depth
...
def intersection_ray_depth(ray: RaySegment, face: Face) -> float:
    ...
    """
    ray_base = np.array(ray.start())
    ray_diretion = ray.direction().get_np_ndarray()
    ray_is_finite = ray.is_finite
    face_p0 = np.array(face[0])
    face_p1 = np.array(face[1])
    face_p2 = np.array(face[2])

    return cpp_intersection_ray_depth(
        ray_base, ray_diretion, ray_is_finite, face_p0, face_p1, face_p2
    )
