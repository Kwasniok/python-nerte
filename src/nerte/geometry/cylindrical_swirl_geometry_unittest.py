...
class SwirlCylindricalRungeKuttaGeometryConstructorTest(BaseTestCase):
...
class SwirlCylindricalRungeKuttaGeometryPropertiesTest(BaseTestCase):
    def setUp(self) -> None:
        self.swirl = 0.1234
        self.geo = SwirlCylindricalRungeKuttaGeometry(
            max_ray_depth=1.0,
            step_size=1.0,
            max_steps=1,
            swirl=self.swirl,
...
class SwirlCylindricalRungeKuttaGeometryIsValidCoordinateTest(BaseTestCase):
    def setUp(self) -> None:
        self.geo = SwirlCylindricalRungeKuttaGeometry(
            max_ray_depth=1.0, step_size=1.0, max_steps=1, swirl=1.0
        )
...
class SwirlCylindricalRungeKuttaGeometryRayFromEuclideanEdgeCaseTest(
    BaseTestCase
):
    def setUp(self) -> None:
        swirl = 1 / 17
        ...
        self.points = (Coordinates3D((2, math.pi / 3, 1 / 5)),)
        self.targets = (Coordinates3D((7, math.pi / 11, 1 / 13)),)
        vectors = (
            AbstractVector(
                (
                    -2 + 7 * math.cos(9 / 1105 - (8 * math.pi) / 33),
                    42 / 1105
                    - 7 / 85 * math.cos(9 / 1105 - (8 * math.pi) / 33)
                    + 7 / 2 * math.sin(9 / 1105 - (8 * math.pi) / 33),
                    -8 / 65,
                )
            ),
        )
        ...
        #   {{2.0, 1.0472, 5.0}, {3.10531, -2.41662, -0.123077}}
        self.initial_tangents = tuple(
            self.geo.normalized(tan) for tan in self.tangents
        )
        # self.initial_tangents numerically:
        #   {{2.0, 1.0472, 5.0}, {0.543923, -0.423292, -0.021558}}
        self.invalid_coords = (
            Coordinates3D((-1.0, 0.0, 0.0)),
            Coordinates3D((math.inf, 0.0, 0.0)),
            Coordinates3D((2.0, -(math.pi + swirl * 2.0 * 3.0 + 1e-9), 3.0)),
            Coordinates3D((2.0, +(math.pi + swirl * 2.0 * 3.0 + 1e-9), 3.0)),
...
class SwirlCylindricalRungeKuttaGeometryEuclideanEdgeCaseVectorTest(
    BaseTestCase
):
    def setUp(self) -> None:
        # coordinates: r, 𝜑, z
...
class SwirlCylindricalRungeKuttaGeometryGeodesicEquationTest(BaseTestCase):
    def setUp(self) -> None:
        self.geo = SwirlCylindricalRungeKuttaGeometry(
            max_ray_depth=1.0, step_size=1.0, max_steps=1, swirl=1.0
        )
...
class SwirlCylindricalRungeKuttaGeometryEuclideanEdgeCaseIntersectsTest(
    BaseTestCase
):
    def setUp(self) -> None:
        # coordinates: r, 𝜑, z
...
class SwirlCylindricalRungeKuttaGeometryIntersectsTest(BaseTestCase):
    def setUp(self) -> None:
        # coordinates: r, 𝜑, z
        ...
        # geometry (cylindrical & non-euclidean)
        geo = SwirlCylindricalRungeKuttaGeometry(
            max_ray_depth=math.inf,
