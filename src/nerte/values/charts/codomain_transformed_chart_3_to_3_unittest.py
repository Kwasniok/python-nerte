...
from nerte.values.domains import OutOfDomainError, R3, CartesianProduct3D
...
from nerte.values.charts.codomain_transformed_chart_3_to_3 import (
    CodomainTransformedChart3DTo3D,
)
...
class LinearTransformationTest(BaseTestCase):
    ...
    def setUp(self) -> None:
        ...
        id_chart = IdentityChart3D(domain1)
        transformation = Linear(domain1, domain2, matrix=matrix)
        # concats: id_chart . transformation
        self.chart = CodomainTransformedChart3DTo3D(transformation, id_chart)
        inside = (-0.5, 0.0, 0.5)
        ...
        self.tangent_deltas_inside_expected = tuple(
            TangentialVectorDelta(mat_vec_mult(matrix, t.vector), ZERO_VECTOR)
        ...
        self.initial_tangents_expected = tuple(
            TangentialVector(
                vector_as_coordinates(
                    mat_vec_mult(matrix, coordinates_as_vector(c1))
                ),
                mat_vec_mult(
                    matrix,
...
class NonLinearTransformationTest(BaseTestCase):
    # pylint: disable=R0902
    def setUp(self) -> None:
        id_chart = IdentityChart3D(R3)
        transformation = CARTESIAN_TO_CYLINDRICAL
        # concats: id_chart . transformation
        self.chart = CodomainTransformedChart3DTo3D(transformation, id_chart)
        self.coords_inside = Coordinates3D((2, 3, 5))
        self.coords_inside_embedded = Coordinates3D(
            (math.sqrt(13), math.atan2(3, 2), 5)
        )
        self.tangential_space_expected = (
            AbstractVector(
                (
        ...
        self.metric_expected = Metric(
            AbstractMatrix(
                UNIT_VECTOR0, AbstractVector((0.0, 13, 0.0)), UNIT_VECTOR2
            )
        )
        # scalar products of basis vectors:
        self.tangent_inside = TangentialVector(
            self.coords_inside, UNIT_VECTOR0
        )  # TODO
        self.length_expected = math.nan  # TODO
        self.tangent_inside_normalized = TangentialVector(
            self.coords_inside, UNIT_VECTOR0  # TODO
        )
        self.tangent_delta_inside_expected = TangentialVectorDelta(
            ZERO_VECTOR, ZERO_VECTOR  # TODO
        )
        self.initial_coords_inside = (
            self.coords_inside,
            Coordinates3D((7, 11, 13)),
        )
        self.initial_tangent_expected = TangentialVector(
            self.coords_inside, AbstractVector((5, 8, 8))
        )
        self.abs_tolerance = 1e-12
    ...
    def test_tangential_space(self) -> None:
        """Tests tangential space."""
        b0, b1, b2 = self.tangential_space_expected
        v0, v1, v2 = self.chart.tangential_space(self.coords_inside)
        self.assertPredicate2(
            vec_almost_equal(delta=self.abs_tolerance), v0, b0
        )
        self.assertPredicate2(
    ...
    def test_metric(self) -> None:
        """Tests metric."""
        metric = self.chart.metric(self.coords_inside)
        self.assertPredicate2(
            metric_almost_equal(delta=self.abs_tolerance),
            metric,
            self.metric_expected,
...
# def test_scalar_product(self) -> None:
#     """Tests scalar product."""
#     for i, v in enumerate(STANDARD_BASIS):
#         for j, w in enumerate(STANDARD_BASIS):
#             val = self.chart.scalar_product(self.coords_inside, v, w)
#             self.assertPredicate2(
#                 scalar_almost_equal(delta=self.abs_tolerance),
#                 val,
#                 STANDARD_BASIS[i][j],
...
# def test_length(self) -> None:
#     """Tests length"""
#     val = self.chart.length(self.tangent_inside)
#     self.assertPredicate2(
#         scalar_almost_equal(delta=self.abs_tolerance),
#         val,
#         self.length_expected,
...
# def test_normalized(self) -> None:
#     """Tests normalized"""
#     tan = self.chart.normalized(self.tangent_inside * 3.3)
#     self.assertPredicate2(
#         tan_vec_almost_equal(delta=self.abs_tolerance),
#         tan,
#         self.tangent_inside,
...
# def test_geodesics_equation(self) -> None:
#     """Tests geodesics_equation"""
#     delta = self.chart.geodesics_equation(self.tangent_inside)
#     self.assertPredicate2(
#         tan_vec_almost_equal(delta=self.abs_tolerance),
#         delta_as_tangent(delta),
#         delta_as_tangent(self.tangent_delta_inside_expected),
#     )
#
# def test_initial_geodesic_tangent_from_coords(self) -> None:
#     """Tests initial geodesic tangent from coordinates."""
